...
Tensor cumprod_backward(const Tensor &grad, const Tensor &input, int64_t dim) {
  ...
  */
  ...
  for (int k = 0; k < dim_size; ++k) {
    ...
    // dim_size - k
    AT_ASSERT(omitted_products.size(dim) == dim_size - k, " Except dim == dim_size - k!");
...
Tensor max_pool_double_backward(const Tensor & grad, const Tensor & indices, int dim) {
  AT_ASSERT(indices.dim() >= dim, "indices.dim() should >= dim");
...
Tensor diag_backward(const Tensor & grad, IntList input_sizes, int64_t diagonal) {
  ...
  AT_ASSERT(ndimension == 1 || ndimension == 2, "diag_backward only support 1/2 dimension");
...
Tensor sum_exclude_dim1(const Tensor& to_sum, bool keepdim) {
...
}} // namespace express::autogenerated
