...
local boxSampling = function(modelInfo, imageWidth, imageHeight, labels) 
    ...
    for i = 1, #modelInfo.boxes do
        local wid = targetWidth - (modelInfo.boxes[i][1] - 1)
        local hei = targetHeight - (modelInfo.boxes[i][2] - 1)
        
        local confTarget = torch.Tensor(modelInfo.classNumber, wid, hei)
    ...
    print(#labels, positiveNumber, negativeNumber)
